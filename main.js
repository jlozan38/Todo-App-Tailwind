...
newBtn.addEventListener("click", () => {
    if (newText.value != "") {
      ...
      newTodo.classList.add("text-center","p-3","bg-white","mt-4","rounded","shadow-lg","cursor-pointer","hover:bg-green-500");
  ...
  const move = (element, destination, convertTo) => {
    if (element.localName == "li") {
      if (convertTo == "completed") {
        element.classList.add("text-red-500", "line-through", "hover:bg-red-500", "hover:text-white");
        element.classList.remove("hover:bg-green-500");
    } else {
      element.classList.remove("text-red-500", "line-through", "hover:bg-red-500", "hover:text-white");
